	...
	def mainThread(self):
		...
		try:
			...
			while(vsiCommonPythonApi.getSimulationTimeInNs() < self.totalSimulationTime):
				...
				global current_x, current_y, current_theta, last_sim_time
				current_sim_time = vsiCommonPythonApi.getSimulationTimeInNs()
				dt = (current_sim_time - last_sim_time) * 1e-9
				last_sim_time = current_sim_time
				if dt > 0:
					v = self.mySignals.cmd_v
					w = self.mySignals.cmd_w
				...
				self.mySignals.cmd_v, receivedData = self.unpackBytes('d', receivedData, self.mySignals.cmd_v)

				signalNumBytes = 8
				receivedData = vsiCanPythonGateway.recvVariableFromCanPacket(signalNumBytes, 0, 64, 21)
				self.mySignals.cmd_w, receivedData = self.unpackBytes('d', receivedData, self.mySignals.cmd_w)
				...
				vsiCanPythonGateway.setCanPayloadBits(self.packBytes('d', self.mySignals.pose_x), 0, 64)
				vsiCanPythonGateway.setDataLengthInBits(64)
				vsiCanPythonGateway.sendCanPacket()

				vsiCanPythonGateway.setCanId(11)
				vsiCanPythonGateway.setCanPayloadBits(self.packBytes('d', self.mySignals.pose_y), 0, 64)
				vsiCanPythonGateway.setDataLengthInBits(64)
				vsiCanPythonGateway.sendCanPacket()

				vsiCanPythonGateway.setCanId(12)
				vsiCanPythonGateway.setCanPayloadBits(self.packBytes('d', self.mySignals.pose_theta), 0, 64)
				vsiCanPythonGateway.setDataLengthInBits(64)
				vsiCanPythonGateway.sendCanPacket()
				...
				print("  Inputs:")
				print("\tcmd_v =", end = " ")
				print(self.mySignals.cmd_v)
				print("\tcmd_w =", end = " ")
				print(self.mySignals.cmd_w)
				print("  Outputs:")
				print("\tpose_x =", end = " ")
				print(self.mySignals.pose_x)
				print("\tpose_y =", end = " ")
				print(self.mySignals.pose_y)
				print("\tpose_theta =", end = " ")
				print(self.mySignals.pose_theta)
				print("\n\n")
