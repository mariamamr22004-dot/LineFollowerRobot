	...
	def mainThread(self):
		...
		try:
			...
			while(vsiCommonPythonApi.getSimulationTimeInNs() < self.totalSimulationTime):
				...
				steering_w = (kp * lateral_error) + (ki * error_sum) + (kd * error_diff) + (0.3 * heading_error)
				self.mySignals.cmd_w = steering_w
				self.mySignals.cmd_v = 0.4
				...
				vsiCanPythonGateway.setCanPayloadBits(self.packBytes('d', self.mySignals.cmd_v), 0, 64)
				vsiCanPythonGateway.setDataLengthInBits(64)
				vsiCanPythonGateway.sendCanPacket()

				vsiCanPythonGateway.setCanId(21)
				vsiCanPythonGateway.setCanPayloadBits(self.packBytes('d', self.mySignals.cmd_w), 0, 64)
				vsiCanPythonGateway.setDataLengthInBits(64)
				vsiCanPythonGateway.sendCanPacket()
